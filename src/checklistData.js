...
const CONDITIONAL_TASKS = [
  ...
  // Arriving in Mexico (international moves only)
  { id: 171, category: 'Admin', monthsBefore: 3,
    label: 'Research the correct Mexican visa or residency permit — Residente Temporal for stays over 180 days, Residente Permanente for long-term; apply at a Mexican consulate before departure',
    condition: a => a.destCountry === 'Mexico' && !isDomestic(a) && !holdsDestPassport(a) },
  { id: 172, category: 'Admin', monthsBefore: -1,
    label: 'Get your CURP (Clave Única de Registro de Población) — Mexico\'s unique population registry code, required for almost all official and everyday services',
    condition: a => a.destCountry === 'Mexico' && !isDomestic(a) },
  { id: 173, category: 'Admin', monthsBefore: -1,
    label: 'Register for your RFC (Registro Federal de Contribuyentes) — Mexican tax ID required for employment, opening a bank account, and many services',
    condition: a => a.destCountry === 'Mexico' && !isDomestic(a) },
  { id: 174, category: 'Admin', monthsBefore: -1,
    label: 'Open a Mexican bank account — BBVA, Santander, and HSBC are common choices for foreigners; bring your passport, CURP, and proof of address',
    condition: a => a.destCountry === 'Mexico' && !isDomestic(a) },
  { id: 175, category: 'Admin', monthsBefore: -1,
    label: 'Register with IMSS (Instituto Mexicano del Seguro Social) if employed, or research private health insurance options if self-employed or still job hunting',
    condition: a => a.destCountry === 'Mexico' && !isDomestic(a) },
  { id: 176, category: 'Admin', monthsBefore: 0,
    label: 'Get a local Mexican SIM card — Telcel and AT&T Mexico are the main carriers with the best national coverage',
    condition: a => a.destCountry === 'Mexico' && !isDomestic(a) },
  { id: 177, category: 'Admin', monthsBefore: -1,
    label: 'Register with the nearest embassy or consulate of your home country in Mexico — important for safety, emergency assistance, and keeping records up to date',
    condition: a => a.destCountry === 'Mexico' && !isDomestic(a) },
  { id: 178, category: 'Admin', monthsBefore: -1,
    label: 'Find a notario público — Mexican notaries have much broader legal powers than in most countries and are required for property purchases, company formation, and many legal processes',
    condition: a => a.destCountry === 'Mexico' && !isDomestic(a) },
